...
class BootStrap {
	...
	public function load_supported_features() {
		...
		new Enqueue_Scripts( $this->module_roots, $utility, $new_features_arr, $plugin_data, $custom_plugin_data );
		...
		if ( is_admin() ) {
			...
			new Settings( $this->module_roots, $plugin_data, $custom_plugin_data, $utility, $new_features_arr, $support_diagnostics );
			...
			new Settings_New_Features( $this->module_roots, $new_features_arr, $plugin_data, $custom_plugin_data, $utility );
		...
		if ( ss_fs()->can_use_premium_code__premium_only() ) {
			...
			$path = $root . 'modules/classes/modules_bootstrap.php';
			if ( file_exists( $path ) ) {
				require_once $path;
				new Modules_BootStrap( $this->module_roots );
			}
		}
